...
#ifndef KALDI_BAUD_HMM_GMM_BOUNDARY_SAMPLER_H
#define KALDI_BAUD_HMM_GMM_BOUNDARY_SAMPLER_H
...
struct HmmGmmBoundarySamplerConfig {
  ...
  HmmGmmBoundarySamplerConfig(BaseFloat cluster_gamma = 1,
...
class HmmGmmBoundarySampler {
public:

  HmmGmmBoundarySampler(const HmmGmmBoundarySamplerConfig &config):
  	config_(config) {
	...
	~HmmGmmBoundarySampler() {
		if (posteriors_writer_.IsOpen()) {
			posteriors_writer_.Close();
		}
	}
                       
  void SetOptions(const HmmGmmBoundarySamplerConfig &config) {
    config_ = config;
    if (posteriors_writer_.IsOpen()) {
    	posteriors_writer_.Close();
    }
  ...
  HmmGmmBoundarySamplerConfig GetOptions() {
    return config_;
  }
	...
	BaseFloat ComputeHmmGmmLikelihood(const SubMatrix<BaseFloat> &features,
																		const AmDiagGmm &am_gmm,
																		const TransitionModel &trans_model,
																		const ContextDependency &ctx_dep,
	...
	private:
		...
		HmmGmmBoundarySamplerConfig config_;
