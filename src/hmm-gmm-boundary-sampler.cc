...
bool HmmGmmBoundarySampler::ResampleAlignment(const Matrix<BaseFloat> &features,
...
void HmmGmmBoundarySampler::GetBoundsFromLandmarks(const Matrix<BaseFloat> features,
																									 const LandmarkSeq &landmarks,
...
void HmmGmmBoundarySampler::GetBoundsFromAlignment(const std::vector<int32> &alignment,
																									const TransitionModel &trans_model,
																									std::vector<int32> *old_bounds,
...
void HmmGmmBoundarySampler::SampleBounds(const Matrix<BaseFloat> &features,
																				const std::vector<int32> &old_bounds, 
																				const LandmarkSeq &landmarks,
																				const AmDiagGmm &am_gmm,
																				const TransitionModel &trans_model,
...
void HmmGmmBoundarySampler::SampleClusters(const Matrix<BaseFloat> &features,
																					const std::string &utt_id,
	...
	for (int32 s = 0; s < num_segs; ++s) {
		...
		for (int32 p = 0; p < num_phones; ++p) {
			...
			if (class_counts[p] > 0) {
				loglike = ComputeHmmGmmLikelihood(segment_features, am_gmm, trans_model, ctx_dep, p);
...
void HmmGmmBoundarySampler::SampleStateSequence(const Matrix<BaseFloat> &features,
																								const std::vector<int32> &new_bounds,
...
BaseFloat HmmGmmBoundarySampler::ComputeSegmentLikelihood(const SubMatrix<BaseFloat> &features,
																													const AmDiagGmm &am_gmm,
																													const TransitionModel &trans_model,
																													const ContextDependency &ctx_dep,
																													const std::vector<int32> &class_counts,
	...
	for (int32 p = 0; p < num_phones; ++p) {
		if (class_counts[p] > 0) {
			const BaseFloat likelihood = ComputeHmmGmmLikelihood(features, am_gmm, trans_model, ctx_dep, p);
...
BaseFloat HmmGmmBoundarySampler::ComputeHmmGmmLikelihood(const SubMatrix<BaseFloat> &features,
																												const AmDiagGmm &am_gmm,
																												const TransitionModel &trans_model,
																												const ContextDependency &ctx_dep,
																												const int32 phone_index) {
...
void HmmGmmBoundarySampler::WriteNewAlignment(const std::vector<int32> &state_sequence,
																							const std::string &utt_id,
																						 	Int32VectorWriter *alignment_writer) {
