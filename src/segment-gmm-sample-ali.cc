// baudbin/segment-gmm-sample-ali.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Gibbs re-sample alignments using Segmental GMM-based models.\n"
        "Usage: segment-gmm-sample-ali [options] tree-in model-in features-rspecifier"
        " landmarks-rspecifier alignments-wspecifier stats-wspecifier\n";

    ParseOptions po(usage);
    Timer timer;
    SegmentGmmBoundarySamplerConfig config;
    ...
    // We read in the transition model and context dep tree, but we ignore them.

    const std::string stats_key = "baud_stats";
    ContextDependency ctx_dep;
    ReadKaldiObject(tree_in_filename, &ctx_dep);

    // We read in the transition model, but we ignore it.
    ...
    KALDI_LOG << "Number of pdfs in am_gmm is " << am_gmm.NumPdfs();
    ...
    SegmentGmmBoundarySampler sampler(config);
    ...
    for (; !features_reader.Done(); features_reader.Next()) {
      ...
      if (alignments_reader.IsOpen()) {
        ...
        sampler.GetBoundsFromAlignment(alignment, &old_bounds, &old_clusters);
      } else {
        sampler.GetBoundsFromLandmarks(features, landmarks, major_only, &old_bounds);
      }
      if (sampler.ResampleAlignment(features, old_bounds, landmarks, am_gmm,
        utt, class_counts, &new_class_counts, &alignments_writer, &like)) {
        tot_like += like;
