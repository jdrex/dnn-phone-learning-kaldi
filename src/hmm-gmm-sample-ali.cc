...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Gibbs re-sample alignments using HMM-GMM-based models.\n"
        "Usage: hmm-gmm-sample-ali [options] tree-in model-in features-rspecifier"
        " landmarks-rspecifier alignments-wspecifier stats-wspecifier\n";

    ParseOptions po(usage);
    Timer timer;
    HmmGmmBoundarySamplerConfig config;
    ...
    HmmGmmBoundarySampler sampler(config);
